...
if st.sidebar.button("🔄 Hot-Reload Core Dataset", use_container_width=True, help="Clears Streamlit memory cache and re-reads the underlying CSV file."):
    st.cache_data.clear()
    st.toast("System cache purged. Re-reading CSV dataset...", icon="🔄")

# Load and execute interface warnings outside execution cache container boundary
if not os.path.exists(DATA_FILE):
    st.sidebar.info("💡 No 'jor_scores.csv' detected. Initialized demo dataset archive.")
...
st.warning("**DATA MODE: HISTORICAL ARCHIVE** (Live Sensor Ingestion: Not Yet Connected)")
...
m1.metric("DATA ENGINE STATUS", "STATIC READ")
...
with col_sidebar:
    ...
    st.text(f"Sensor Telemetry Factor (P):  {case_row['P']:.2f}")
...
with col_charts:
    ...
    with tab_frontier:
        ...
        ax.set_ylabel('Kinetic Behavior Severity (Flight_Mod)', color='#cccccc')
    ...
    with tab_quadrant:
        ...
        ax.axhline(0.75, color='#ff453a', linestyle='--', label='Critical Threshold Limit (0.75)')
    ...
    with tab_uncertainty:
        ...
        if 'CI_2.5%' in filtered_df.columns and 'CI_97.5%' in filtered_df.columns:
            ...
            st.warning("Historical MCMC post-processing parameters missing from dataset.")
