...
# --- START OF AEROSPACE SAFETY & RISK ASSESSMENT LOGIC ---
    # We define 'Aero_Safety_Risk' as the probability of a physical object (SOP) 
    # presence, boosted by the Flight_Mod (kinetic/maneuver hazard).
    if 'Flight_Mod' in df_merged.columns and 'SOP_Mean' in df_merged.columns:
        # Kinetic Multiplier: Mean Solid Object Probability * (1 + Flight Anomaly)
        df_merged['Aero_Safety_Risk'] = (df_merged['SOP_Mean'] * (1 + df_merged['Flight_Mod'])).round(3)
...
# Round results for the final CSV save
    # ADDED 'SOP_Mean' and 'Aero_Safety_Risk' to this list
    decimal_cols = ['SOP_Mean', 'Posterior_Mean', 'CI_2.5%', 'CI_97.5%', 'Aero_Safety_Risk']
    for col in decimal_cols:
        if col in df_merged.columns:
            df_merged[col] = df_merged[col].round(3)

    df_merged.to_csv(input_csv, index=False)
    print(f"\nSuccess! Bayesian inference & Safety Audit complete. CSV updated: {input_csv}")
