...
# --- UPDATED EXTRACTION LOGIC ---
    # We must extract the SOP samples to calculate the mean for safety risk assessments
    sop_samples = trace.posterior['SOP'].values.reshape(-1, num_cases)
    post_samples = trace.posterior['Posterior_NH'].values.reshape(-1, num_cases)

    df_results = pd.DataFrame({
        'case_name': df['case_name'],
        'SOP_Mean': np.mean(sop_samples, axis=0),  # Required for Aerospace Safety Logic
        'Posterior_Mean': np.mean(post_samples, axis=0),
        'CI_2.5%': np.percentile(post_samples, 2.5, axis=0),
        'CI_97.5%': np.percentile(post_samples, 97.5, axis=0)
    })
